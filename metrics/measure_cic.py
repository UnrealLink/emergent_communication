    ...
    for _ in range(args.horizon):
       ...
       # get speaker message
        ...
        for m in range(args.vocab):
            pm = torch.exp(comm_logp[0][m]).detach().cpu().numpy()
            ...
            pam = torch.exp(fake_logp[0][action]).detach().cpu().numpy() * pm
            pa += pam
            pams.append(pam)
            pms.append(pm)
        for pam, pm in zip(pams, pms):
            if pam * pa * pm != 0:
                CIC += pam * np.log(pam / (pa * pm)) / args.horizon
            else:
